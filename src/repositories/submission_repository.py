...
from ..domain.models import StudentSubmission, Turma
...
class SubmissionRepository:
    ...
    def get_submissions_for_assignment(self, turma_name: str, assignment_name: str) -> List[StudentSubmission]:
        ...
        for student_dir in assignment_submissions_path.iterdir():
            if student_dir.is_dir():
                submission = self._load_submission(student_dir, turma_name, assignment_name)
                if submission:
                    submissions.append(submission)
        
        return submissions
    
    def get_submission(self, turma_name: str, assignment_name: str, student_name: str) -> Optional[StudentSubmission]:
        """Retorna uma submissão específica."""
        turma_path = self.respostas_path / turma_name
        assignment_submissions_path = turma_path / f"{assignment_name}-submissions"
        student_path = assignment_submissions_path / student_name
        
        if student_path.exists() and student_path.is_dir():
            return self._load_submission(student_path, turma_name, assignment_name)
        return None
    ...
    def _load_turma(self, turma_path: Path) -> Optional[Turma]:
        ...
        for assignment_dir in turma_path.iterdir():
            if assignment_dir.is_dir() and assignment_dir.name.endswith('-submissions'):
                ...
                # Coleta nomes dos alunos
                for student_dir in assignment_dir.iterdir():
                    if student_dir.is_dir():
                        students.add(student_dir.name)
    ...
    def _load_submission(self, submission_path: Path, turma_name: str, assignment_name: str) -> Optional[StudentSubmission]:
    ...
    def get_student_submissions(self, turma_name: str, student_name: str) -> List[StudentSubmission]:
        """Retorna todas as submissões de um aluno específico."""
        submissions = []
        turma_path = self.respostas_path / turma_name
        ...
        for assignment_dir in turma_path.iterdir():
            if assignment_dir.is_dir() and assignment_dir.name.endswith('-submissions'):
                ...
                student_path = assignment_dir / student_name
                
                if student_path.exists() and student_path.is_dir():
                    submission = self._load_submission(student_path, turma_name, assignment_name)
                    if submission:
                        submissions.append(submission)
